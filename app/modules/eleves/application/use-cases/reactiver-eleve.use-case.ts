...
export class ReactiverEleveUseCase
  implements UseCase<{ eleveId: string; etablissementId: string }, EleveReadDto>
{
  constructor(
  ...
  async execute(params: { eleveId: string; etablissementId: string }): Promise<EleveReadDto> {
    ...
    const eleveMisAJour = eleve.reactiver(this.clock.maintenant());
